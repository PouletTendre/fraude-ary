...
async def list_transactions(
    ...
    base_query = select(Transaction).where(Transaction.user_email == current_user.email)
    ...
    result = await db.execute(
        base_query.order_by(Transaction.date.desc()).limit(limit).offset(offset)
    )
    transactions = result.scalars().all()

    return PaginatedResponse(
        data=[_tx_to_response(t) for t in transactions],
        total=total,
        limit=limit,
        offset=offset,
    )
